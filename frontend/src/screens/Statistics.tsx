...
export function Statistics() {
  ...
  // Filters state (Defaults to 40 days range ending today for nice sample charts)
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] p-4 md:p-6 space-y-6 select-none">
      ...
      {/* Width limited layout that stretches properly on big screens */}
      <div className="max-w-7xl mx-auto w-full space-y-6">
        
        {/* GREEN BADGE/TAB */}
        <div className="flex">
          <div className="bg-[#8CC63F] text-white px-5 py-2.5 rounded-t-xl font-extrabold text-xs flex items-center gap-2 shadow-xs">
        ...
        {/* PRIMARY CONTAINER */}
        <div className="bg-white rounded-b-xl rounded-tr-xl shadow-md p-6 border border-gray-200/80 space-y-6">
          
          {/* HORIZONTAL DATE RANGE FILTERS */}
          <div className="flex flex-col sm:flex-row items-end gap-3 max-w-xl">
            
            {/* Fecha inicial */}
            <div className="flex-1 min-w-[140px] space-y-1">
              <label className="text-gray-700 font-extrabold text-[10px] block uppercase tracking-wide">
                Fecha inicial
              </label>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-white border border-gray-300 rounded-lg px-3.5 py-1.5 text-xs font-bold text-gray-800 outline-none focus:border-[#8CC63F] transition-all cursor-pointer"
                />
              </div>
            </div>
            ...
            <button
              ...
              className="bg-[#8CC63F] hover:bg-[#7bb335] active:scale-95 text-white p-2 rounded-lg shadow-sm transition-all flex items-center justify-center cursor-pointer h-[32px] w-[32px] shrink-0"
        ...
        {/* METRICS BRIEFING STATS CARDS & RANKING DESEMPEÑO */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          
          <div className="lg:col-span-2 grid grid-cols-2 gap-4">
            {/* Recaudo Card */}
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200/60 flex flex-col justify-between">
          ...
          {/* DESEMPEÑO RANKING */}
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-200/60 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xs font-black text-[#333333] uppercase tracking-wider">Desempeño</h3>
                <p className="text-[9px] text-gray-400 font-bold mt-0.5">Control de cobro del período</p>
              </div>
            ...
            {topCollectors.length === 0 ? (
              ...
              <div className="space-y-3">
                {topCollectors.map((collector, idx) => {
                  ...
                  return (
                    <div key={collector.name} className="space-y-1 bg-gray-50/50 border border-gray-100 p-2.5 rounded-xl">
                      <div className="flex justify-between items-center text-xs">
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs">{medal}</span>
                          <span className="font-extrabold text-gray-700 text-[11px] truncate max-w-[100px]">
                            {collector.name}
                          </span>
                        </div>
