...
export function BCApprovals({ onNavigate }: BCApprovalsProps) {
  const [activeTab, setActiveTab] = useState<'pending' | 'history' | 'boxes'>('pending');
  ...
  // Load real-time closed boxes waiting for confirmation
  useEffect(() => {
    if (!tenantId) return;
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const loaded: Box[] = snapshot.docs.map(d => ({
        id: d.id,
        ...(d.data() as any)
      }));
      setClosedBoxes(loaded);
    }, (error) => {
      console.error("Error loading closed boxes for confirmation:", error);
    });

    return () => unsubscribe();
  }, [tenantId]);

  const handleConfirmBoxClick = async (boxId: string, collectorName: string) => {
    setConfirmingBoxId(boxId);
    try {
      // Usa estritamente a API do backend POST /api/boxes/confirm
      await confirmBoxByAdmin(boxId, tenantId);
      setSuccessMessage(`¡Caja de ${collectorName} CONFIRMADA correctamente!`);
      setTimeout(() => setSuccessMessage(null), 4000);
    } catch (error: any) {
      console.error("Error confirming box via API:", error);
      alert(error.message || "Error al confirmar la caja.");
    } finally {
      setConfirmingBoxId(null);
    }
  };
      ...
      <div className="p-3">
        <div className="flex flex-col sm:flex-row gap-3 mb-4 bg-white p-2 rounded border border-gray-300">
          ...
          <div className="flex flex-1 rounded bg-gray-100 p-1">
            ...
            <button
              id="tab-boxes-approvals"
              onClick={() => setActiveTab('boxes')}
              className={`flex-1 py-1.5 text-xs font-bold rounded text-center transition-all ${
                activeTab === 'boxes' 
                  ? 'bg-[#6A008A] text-white shadow-sm' 
                  : 'text-gray-600 hover:text-[#6A008A]'
              }`}
            >
              Caixas para Confirmação ({closedBoxes.length})
            </button>
          </div>
        ...
        {activeTab === 'boxes' ? (
          closedBoxes.length === 0 ? (
            <div className="bg-white border border-gray-300 p-8 text-center text-xs text-gray-500 rounded">
              Não há caixas fechadas aguardando confirmação.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {closedBoxes
                .filter(b => (b.userName || '').toLowerCase().includes(searchQuery.toLowerCase()))
                .map(box => (
                  <div key={box.id} className="bg-white border border-yellow-300 border-l-4 border-l-yellow-500 rounded-sm shadow-sm p-4 flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start border-b border-gray-100 pb-2 mb-2">
                        <div className="flex items-center gap-1.5">
                          <Archive className="w-4 h-4 text-yellow-600" />
                          <div>
                            <h4 className="text-xs font-bold text-gray-800 leading-none">{box.userName || 'Cobrador'}</h4>
                            <span className="text-[9px] text-gray-500">{box.cnName || 'Sociedade'}</span>
                          </div>
                        </div>
                        <span className="text-[10px] bg-yellow-100 text-yellow-800 font-bold px-2 py-0.5 rounded">Aguardando Confirmação</span>
                      </div>

                      <div className="space-y-1.5 text-xs my-3">
                        <div className="flex justify-between">
                          <span className="text-gray-500">Saldo Inicial:</span>
                          <span className="font-bold text-gray-800">${fmtCents(box.initialAmount || 0)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Total Cobrado:</span>
                          <span className="font-bold text-green-700">+${fmtCents(box.totalCollections || 0)}</span>
                        </div>
                        <div className="flex justify-between border-t border-gray-100 pt-1">
                          <span className="text-gray-700 font-bold">Saldo Final Fechado:</span>
                          <span className="font-black text-[#6A008A] text-sm">${fmtCents(box.finalAmount || 0)}</span>
                        </div>
                      </div>
                    </div>

                    <button
                      onClick={() => handleConfirmBoxClick(box.id, box.userName || 'Cobrador')}
                      disabled={confirmingBoxId === box.id}
                      className="w-full bg-[#16A34A] hover:bg-[#15803D] text-white font-bold py-2 rounded text-xs flex items-center justify-center gap-1.5 transition-colors disabled:opacity-50"
                    >
                      <Check className="w-4 h-4" />
                      {confirmingBoxId === box.id ? 'Confirmando...' : 'Confirmar Caixa (BFF API)'}
                    </button>
                  </div>
                ))}
            </div>
                      ...
                      <div className="flex justify-between items-start border-b border-gray-100 pb-2 mb-2">
                        <div className="flex items-center gap-1.5">
                          ...
                          <div>
                            ...
                            <span className="text-[9px] text-gray-500">{req.cnName || 'Sociedade Principal'}</span>
                          </div>
                        </div>
