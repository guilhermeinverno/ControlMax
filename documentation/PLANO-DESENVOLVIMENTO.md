...
**Versão:** 1.1  
**Data:** 27/08/2026  
**Base:** [`PENDENCIAS-DESENVOLVIMENTO.md`](./PENDENCIAS-DESENVOLVIMENTO.md) · [`guia-controlmax.md`](./guia-controlmax.md) · [`ARQUITETURA-SSOT.md`](./ARQUITETURA-SSOT.md)  
...
**Fontes:** [`14-roadmap.md`](../docs/controlmax/14-roadmap.md) · [`backlog-piloto.md`](../docs/comparativo/backlog-piloto.md) · [`guia-controlmax.md`](./guia-controlmax.md)
...
Fase 6      Enterprise (Zod ✅ → Ledger sombra → RL)   🟡 ENT-01 feito
Fase 7+     Claims revogação / Sync testes / audit BI  ⬜
...
| **6** | Zod + ledger sombra + rate limit | 🟡 `ENT-01` ✅ · `ENT-02`/`ENT-03` ⬜ |
...
| **Esforço** | L–XL |
| **Depende** | `ENT-01` |
| **Trabalho** | Coleção de eventos imutáveis `{ tenantId, transactionId, debitAccount, creditAccount, amountCents, timestamp }` após cada mutação BFF; **sem** trocar ainda o saldo por documento; relatório sombra vs `boxes`/`sales`. |
| **DoD** | 100% das ops financeiras geram evento; inconsistências = 0 em amostra de homolog. |
...
| **ID** | `ENT-03` |
| **Esforço** | M |
| **Trabalho** | Limitar `/api/gemini/assistant`, `/api/transactions/*`, `/api/boxes/*` (in-memory → Redis se multi-instância). |
| **DoD** | 429 documentado; smoke de abuso no assistente e em collection. |
...
|----|---------|---------|----------------|
| `ENT-04` | Revogação claims / force refresh | M | Após mudança de role, forçar refresh de token / invalidar sessão (guia §5.4). |
| `ENT-05` | Testes conflito SyncManager | M | Duplo enqueue, offline→online, 5xx→retry; suite automatizada. |
...
- [ ] `ENT-03` Rate limit
- [ ] `ENT-04` Revogação claims
- [ ] `ENT-05` Testes Sync conflito
...
1. **Ops/QA:** Gate (deploy → QA regressivo → SYNC-01).  
2. **Dev:** `ENT-02` ledger sombra (ou `ENT-03` rate limit em paralelo).
